...
int main()
{
	...
	try
	{
		message_queue input(open_only, "input_Sam");
		message_queue output(open_only, "output_Sam");	
		while(true)
		{
			reset();
			setup();
			...
			while(true)
			{
				//Choice a Position
				choice = decision();
				cout << "Picking: " << (char)(choice.letter+64) << choice.number << endl;
...
position decision()
{
	...
	for(int i=0;i<SIZE;i++)
	{
		for(int j=0;j<SIZE;j++)
		{
			if(theirs[i][j]==2)
			{
				if(i>0 && theirs[i-1][j]==0)
				{
					output.letter = i;
					output.number = j+1;
					return output;
				}
				if(j>0 && theirs[i][j-1]==0)
				{
					output.letter = i+1;
					output.number = j;
					return output;
				}
				if(i<SIZE-1 && theirs[i+1][j]==0)
				{
					output.letter = i+2;
					output.number = j+1;
					return output;
				}
				if(j<SIZE-1 && theirs[i][j+1]==0)
				{
					output.letter = i+1;
					output.number = j+2;
					return output;
	...
	*/
	...
	//smart ship search
	for(int i=0;i<SIZE;i++)//check for alone hits
	{
		for(int j=0;j<SIZE;j++)
		{
			if(theirs[i][j]==2)
	...
	for(int i=0;i<5;i++)
	{
		...
		if(theirs[(int)(j/10)][j%10]==0)
		{
			output.letter = (int)(j/10)+1;
			output.number = j%10+1;
			return output;
		}
	}*/
	
	//smarter linear search
	for(int h=0;h<4;h+=4)
	{
		for(int i=0;i<SIZE;i++)
		{
